<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateProductsTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('products', function (Blueprint $table) {
           $table->increments('id');
           $table->string('pro_name')->nullable();
           $table->string('pro_slug')->nullable();
           $table->unsignedInteger('pro_category_id')->index();
           $table->float('pro_price')->default(0);
           $table->tinyInteger('pro_sale')->default(0);
           $table->tinyInteger('pro_active')->default(1)->index();
           $table->unsignedInteger('pro_author_id')->default(0)->index();
            $table->tinyInteger('hot')->default(0);
            $table->integer('pro_view');
            $table->string('pro_description')->nullable();
            $table->string('pro_avatar');
            $table->string('pro_description_seo')->nullable();
            $table->string('pro_keywork_seo')->nullable();
           $table->timestamps();
           $table->foreign('pro_category_id')->references('id')->on('catrgories');
        });
    }
    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('products');
    }
}
